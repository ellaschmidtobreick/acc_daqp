...
with torch.no_grad():
    for i,batch in enumerate(test_loader):
        ...
        preds = (output.squeeze() > config.t).long() # original prediction #config.t
        # add a new class of constraints that should get ignored
        #preds = torch.where(output.squeeze() < 0.00001, torch.tensor(4), torch.where(output.squeeze() > 0.4, torch.tensor(1),torch.tensor(0))) #config.t
        ...
        # true active set (given by test dataset)
        W_true = (batch.y.numpy()[n:] != 0).astype(int).nonzero()[0]
        # constraints predicted as active and therefore the predicted active constraints
        predicted_active_constraints = (preds.flatten()[n:] == 1)
        W_pred = torch.nonzero(predicted_active_constraints, as_tuple=True)[0].numpy()
        print(f"W_true {W_true}")
        print(output[W_true+n].numpy())
        print(f"W_pred {W_pred}")
        print(output[W_pred+n].numpy())
        
        # solve reduced QP with active constraints with own solver
        ...
        # by prediction the system can be overdetermined: remove as active predicted constraints from the working set until the problem is feasible
        exitflag = -6
        while exitflag == -6:
...
plt.hist(prediction_time, bins=50,range=(np.min(prediction_time),0.2), alpha=0.7, label='prediction time', color='green')
plt.show()
