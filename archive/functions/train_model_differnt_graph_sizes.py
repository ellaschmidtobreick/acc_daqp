...
# n_train = np.full(4000,n)
# n_val = np.full(500,n)
# m_train = np.full(4000,m)
# m_val = np.full(500,m)
...
graph_train, n_train, m_train = generate_qp_graphs_different_sizes(n_min,n_max,m_min,m_max,nth,seed,data_points, "train",H=H,A=A)
graph_val,n_val, m_val = generate_qp_graphs_different_sizes(n_min,n_max,m_min,m_max,nth,seed,data_points, "val",H = H,A= A)
...
model = GNN(input_dim=4, output_dim=1,layer_width = 128)  #input dimension: number of node features # Output dimension 1 for binary classification
...
#while acc != 1:
    ...
    for i in range(len(n_val)):
        ...
        #print(len(val_all_labels), len(val_preds),n+m)
        ...
        val_all_labels = val_all_labels[n+m:]
        val_preds = val_preds[n+m:]

    print(val_wrongly_pred_nodes_graph_i_save)
    # Compute average over graphs
    ...
    if track_on_wandb == True:
        run.log({"acc_train": acc,"acc_val": val_acc,"loss_train": train_loss, "loss_val": val_loss, "prec": prec, "rec": rec, "f1": f1, "acc_graph": acc_graph, "acc_graph_val": acc_graph_val,"perc_wrong_pred_nodes_per_graph":val_mean_wrongly_pred_nodes_per_graph,"num_wrong_pred_nodes_per_graph":np.mean(val_wrongly_pred_nodes_graph_i_save), "threshold": t})
